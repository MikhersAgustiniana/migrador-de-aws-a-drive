...
# CREAR CARPETA
# def crear_carpeta(nombre_carpeta:str,id_folder:str):
#     credenciales = login()
...
def crear_carpeta(nombre_carpeta: str, id_folder: str):
    ...
    # Verificar si la carpeta ya existe dentro del directorio especificado
    query = f"'{id_folder}' in parents and title = '{nombre_carpeta}' and mimeType = 'application/vnd.google-apps.folder' and trashed = false"
    carpeta_existente = credenciales.ListFile({'q': query}).GetList()
    
    if not carpeta_existente:  # Si la lista está vacía, la carpeta no existe
        folder = credenciales.CreateFile({
            'title': nombre_carpeta,
            'mimeType': 'application/vnd.google-apps.folder',
            'parents': [{"kind": "drive#fileLink", 
